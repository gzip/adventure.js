    ...
    return {
        ...
        extend : function(target, source /*, methods, methods, methods...*/)
        {
            ...
            util.each(methods, function(m) {
                util.merge(target[proto], m, {shallow: true});
            });
        },
        ...
        /**
         * Modify a string of text to use camelCase, e.g. image-rendering becomes imageRendering.
        ...
        {
            if (isStr(str) && str.indexOf('-' !== -1)) {
                var parts = str.split('-');
                str = parts.shift();
                util.each(parts, function eachPart(part) {
                    str += util.capitalize(part);
                });
            }
            return str;
        },
        */
         ...
         * @param  {string} [prop=null] CSS property to test.
        ...
        /**
         * Get the standard or vendor property value.
         * @param  {string} prop Standard CSS property to test.
         * @param  {string} prop Standard CSS property value in dash-case.
         * @return {string} Resolved value.
         */
        resolvePropertyValue : function(prop, val)
        {
            var prefixed = util.getVendorPrefix(null, true) + prop,
                exception = util.get(vendorExceptions, util.getVendorPrefix() + '.' + val),
                supports = util.get(win, 'CSS.supports', function(){}),
                result = exception || val;

            if (supports(util.resolvePrefix(prop), prefixed)) {
                result = prefixed;
            }
